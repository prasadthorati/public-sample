...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .teams-bubble { background: #f5f5f5; border: 1px solid #e1e1e1; border-radius: 8px; padding: 14px 16px; margin-bottom: 14px; }
        .teams-bubble-header { font-size: 12px; color: #888; margin-bottom: 8px; font-weight: 600; }
        .teams-bubble-body { font-size: 14.5px; line-height: 1.55; color: #222; }
        .teams-bubble-body table { margin-top: 10px; font-size: 13px; }
        .teams-bubble-body th, .teams-bubble-body td { padding: 5px 10px; }
        .mention-chip { background: #e8e8f7; color: #464775; padding: 1px 7px; border-radius: 10px; font-weight: 600; }
    </style>
    <script>
...
- CHG0690343 – Sat 04 July</textarea>
            ...
            <div class="checkbox-row">
                <input type="checkbox" id="auto_mention_emails" name="auto_mention_emails" checked>
                <label for="auto_mention_emails">Auto-highlight any email address typed directly in the message above (e.g. "cc: name@company.com") as a real @mention</label>
            </div>
        ...
        <p class="hint">Uses the token saved in Step 0. "Send Actual Messages" will ask you to confirm before it sends anything. "Dry Run" also checks (read-only) whether each mention can actually be highlighted, and tells you why if it can't — usually a wrong/typo'd email, or the token's account lacking permission to look up other users in the directory.</p>
    </div>

    {% if previews %}
    <div class="box">
        <h2>Teams Preview (what will actually appear in each chat)</h2>
        {% for p in previews %}
        <div class="teams-bubble">
            <div class="teams-bubble-header">Chat ID: {{ p.chat_id }}</div>
            <div class="teams-bubble-body">{{ p.html|safe }}</div>
        </div>
        {% endfor %}
    </div>
    {% endif %}
    ...
    <div class="box">
        <div class="result-header">
            <h2 style="margin-bottom:0;">Result / Log</h2>
            <div>
...
def find_emails_in_text(text):
    """Return unique emails found in text, in first-seen order."""
    seen = set()
    ordered = []
...
def build_message(rows, message_text, use_everyone, include_table, poc_list=None, notify_poc=False,
                   auto_mention_emails=False, token=None):
    """Returns (message_html, mention_map).
    mention_map: {mention_id: {"type": "everyone"}} or {"type": "user", "name":..., "email":...}

    If auto_mention_emails is True, any email address typed directly in the message text
    (e.g. "cc: name@company.com") is turned into a real @mention in place, instead of staying
    as plain text.
    """
    ...
    if auto_mention_emails:
        for email in find_emails_in_text(message_text):
            if email not in message_html:
                continue  # already consumed/replaced by something else
            resolved, _reason = resolve_user(email, token) if token else (None, "No token set")
            display_name = resolved["displayName"] if resolved else email
            message_html = message_html.replace(email, f'<at id="{next_id}">{html.escape(display_name)}</at>', 1)
            mention_map[next_id] = {"type": "user", "name": display_name, "email": email}
            next_id += 1
...
def render_teams_preview(message_html, mention_map, token):
    """Simulates exactly what the message will look like once delivered in Teams:
    resolved mentions become highlighted chips, unresolved ones fall back to plain text
    (matching the same fallback post_message uses when actually sending)."""
    preview = message_html
    for mention_id, info in mention_map.items():
        if info["type"] == "everyone":
            tag = f'<at id="{mention_id}">Everyone</at>'
            preview = preview.replace(tag, '<span class="mention-chip">@Everyone</span>')
        else:
            tag = f'<at id="{mention_id}">{html.escape(info["name"])}</at>'
            resolved, _reason = resolve_user(info["email"], token) if info["email"] else (None, None)
            if resolved and resolved.get("id"):
                preview = preview.replace(tag, f'<span class="mention-chip">@{html.escape(info["name"])}</span>')
            else:
                preview = preview.replace(tag, html.escape(info["name"]))
    return preview
...
def send_route():
    ...
    try:
        ...
        auto_mention_emails = "auto_mention_emails" in request.form
        ...
        output = [
            ...
            f"POC mention (from Excel columns): {'Enabled' if notify_poc else 'Disabled'}",
            f"Auto-mention emails typed in message: {'Enabled' if auto_mention_emails else 'Disabled'}",
        ...
        for i, (chat_id, rows_full) in enumerate(groups, start=1):
            ...
            message_html, mention_map = build_message(
                rows, message_text, use_everyone, include_table, poc_list, notify_poc,
                auto_mention_emails=auto_mention_emails, token=token
            )
            ...
            if notify_poc:
                if poc_list:
                    output.append("POC(s) from Excel in this group: " + ", ".join(p["name"] for p in poc_list))
                else:
                    output.append("POC(s) from Excel in this group: none found (add POC Name/POC Email columns in Excel)")
            if include_table:
                output.append(rows.to_string(index=False))

            preview_html = render_teams_preview(message_html, mention_map, token)
            previews.append({"chat_id": chat_id, "html": preview_html})

            if dry_run:
                output.append("DRY RUN ONLY - not sent")
                if any(info["type"] == "user" for info in mention_map.values()):
                    output.append("Checking whether each mention can be resolved to a real highlighted mention (read-only check, nothing is sent):")
