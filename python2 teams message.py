...
- CHG0690343 – Sat 04 July</textarea>
        ...
        <p class="hint">Uses the token saved in Step 0. "Send Actual Messages" will ask you to confirm before it sends anything. If a POC email can't be resolved to a Teams account, their name still shows in the message as plain text, and Everyone is still notified.</p>
    </div>
...
def resolve_user(email, token):
    """Look up a user's Graph object id/displayName by email, so they can be @mentioned. Cached."""
    if not email:
        return None
    key = email.lower()
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            result = {"id": data.get("id"), "displayName": data.get("displayName") or email}
        else:
            result = None
...
def post_message(chat_id, message_html, token, mention_map):
    ...
    for mention_id, info in mention_map.items():
        if info["type"] == "everyone":
            ...
            resolved = resolve_user(info["email"], token) if info["email"] else None
            if resolved and resolved.get("id"):
                ...
                # the message still sends (with the name visible, just not a clickable mention).
    ...
    try:
        ...
        for i, (chat_id, rows_full) in enumerate(groups, start=1):
            ...
            if dry_run:
                ...
                try:
                    post_message(chat_id, message_html, token, mention_map)
                    success += 1
