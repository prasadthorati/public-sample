...
# Features:
# - Upload Excel
# - Paste fresh Microsoft Graph token
...
<body>
    ...
    <div class="box">
        ...
        <form method="post" enctype="multipart/form-data" action="/update-chat-ids">
            ...
            <label>Paste fresh Microsoft Graph token</label>
            <textarea class="token" name="token" placeholder="Paste token here" required></textarea>

            <button type="submit">Update Chat IDs in Excel</button>
        </form>
        <p class="hint">Excel should have App ID, App Name, CTASK ID, CTASK Group, Group Name/Teams Group Name, and Chat ID columns. Chat ID can be blank.</p>
    </div>
...
@app.route("/")
def home():
    return render_template_string(HTML, message=None, download_excel=False, download_discovered=False)
...
def update_route():
    ...
    try:
        ...
        discovered, msg = update_chat_ids(excel_path, token)
        LAST_DISCOVERED_FILE = discovered
        msg += "\n\nNext: write full message in one box, choose table/delay, then click Dry Run."
        return render_template_string(HTML, message=msg, download_excel=True, download_discovered=True)
    except Exception as e:
        return render_template_string(HTML, message=f"ERROR:\n{e}", download_excel=False, download_discovered=False)
